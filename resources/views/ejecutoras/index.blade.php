                ...
                <div class="col-lg-12">
                    ...
                    <div class="card">
                        <div class="card-body">                        
                            ...
                            @can('crear_tickets')
                            <a class="btn btn-warning" href="{{ route('executor.create')}}">Nuevo</a>
                            @endcan
                                ...
                                <tr>
                                            <td style="display: none;">{{ $ejecutora->id }}</td>
                                            <td>{{ $ejecutora->nombre }}</td>
                                            <td>{{ $ejecutora->descripcion }}</td>
                                                                             
                                            <td>
                                                @can('editar_tickets')
                                                    <a class="btn btn-info" href="{{ route('executor.edit', $ejecutora->id) }}">Editar</a>
                                                @endcan
                                                @can('borrar_tickets')
                                                <form action="{{ route('executor.destroy', $ejecutora->id) }}" method="POST" style="display: inline;">
                                                    @csrf
                                                    @method('DELETE')
